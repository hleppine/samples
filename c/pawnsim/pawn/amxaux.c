...
/*
 * AALTO-2 SECTION
 * Specific modifications for Aalto-2
 * File I/O is wrapped to on-board filesystem.
 * Note: "malloc" and "free" are also removed from below code.
 */
#ifdef AALTO2_OBC

// TODO: include coffee filesystem?

// TODO: Wrappers for fopen, fread, fclose, rewind
...
int AMXAPI aux_LoadProgram(AMX *amx, const char *filename, void *memblock)
{
  ...
  didalloc = 0;
  if (memblock == NULL) {
	/* MODIFICATION FOR AALTO-2: malloc is not used, AMX_ERR_MEMORY returned */
  ...
  if (result != AMX_ERR_NONE && didalloc) {
    /* MODIFICATION FOR AALTO-2: free is not used */
#if 0
    free(memblock);
#endif
